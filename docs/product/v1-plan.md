...
- `042b` reste bloque tant que les gates pre-code `042a` ne sont pas signes et merges, tant que provider-readiness, signatures humaines, exact model id / snapshot, privacy/legal, region, retention, training/non-training, logging, cout, latence, quotas RPM/TPM, budget cap, kill switch, log hygiene, golden set autoritatif, validator et gate reseau restent incomplets ou `PENDING_ACCOUNT_PROOF`. Les artefacts candidats `042a2a2a`, le pack de cas candidats `042a2` et le pack de double revue aveugle `042a2` ne debloquent ni runtime provider ni appel reseau.
- `042b0` documente OpenAI API comme provider candidat uniquement pour un futur pilote IA runtime : endpoint candidat `/v1/chat/completions`, domaine candidat `eu.api.openai.com`, modele candidat public `gpt-5.4-mini`, exact model id / snapshot exact `PENDING_ACCOUNT_PROOF`. Aucun fallback automatique vers alias, autre modele, autre region ou autre provider n'est autorise. `042b0` ne livre aucun runtime, adapter, SDK, secret, `.env`, appel IA, prompt runtime actif, golden set promu, production ou spec `043`, et ne debloque pas `042b`.
