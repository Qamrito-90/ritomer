...
- `042b` reste bloque tant que les gates pre-code `042a` ne sont pas signes et merges, tant que provider/modele/region/retention/training/cout/latence/quotas restent `NON_DÉTERMINÉ`, tant que la semantic readiness `042a2a1` et le manifeste de perimetre pilote ne sont pas approuves, tant que le golden set et le validator ne sont pas crees dans une mission separee, et tant que le contrat `mapping-suggestion-v1` ou une future version contractuelle n'a pas de decision explicite pour representer ou exclure les outcomes, reason codes et etats de degradation.
