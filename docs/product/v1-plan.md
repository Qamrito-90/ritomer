...
### Rappels
- `042-controlled-ai-mapping-runtime-pilot-v1` est active en SPEC_CREATION DOCS_ONLY pour cadrer le premier pilote IA runtime reel strictement limite aux suggestions de mapping sur dossier demo synthetique. `042a1` ajoute uniquement un gate pack draft de gouvernance/readiness (`PENDING_EVIDENCE` ou `DRAFT`) avant tout code provider `042b`, sans runtime, provider, backend, frontend, DB/migration, OpenAPI, CI, secret, `.env`, appel reseau IA, production, donnee cliente reelle ou spec `043`.
...
- `042b` reste bloque tant que les gates pre-code `042a` ne sont pas signes et merges, tant que provider/modele/region/retention/training/cout/latence/quotas restent `NON_DÉTERMINÉ`, et tant que le contrat `mapping-suggestion-v1` n'a pas de decision explicite pour representer ou exclure `abstention` et `uncertainty`.
