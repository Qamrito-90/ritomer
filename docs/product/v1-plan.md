...
- `042b0` documente OpenAI API comme provider candidat uniquement pour un futur pilote IA runtime : endpoint candidat `/v1/chat/completions`, domaine candidat `eu.api.openai.com`, modele candidat public `gpt-5.4-mini`, exact model id / snapshot exact candidat visible `gpt-5.4-mini-2026-03-17`. `042b0b` ajoute les preuves manuelles OpenAI Platform/API : projet dedie `ritomer-dev`, billing API active, credits `$10`, auto recharge `OFF`, project spend limit `$10`, spend alert `100 % / $10`, modeles autorises `gpt-5.4-mini` et `gpt-5.4-mini-2026-03-17`, API key created: `NOT_CREATED_USER_CONFIRMED`, API key shared: `NO`, et aucun appel reseau IA effectue. La cle API devra etre creee seulement plus tard, au moment d'un gate reseau provider signe, et ne devra jamais etre commise, collee dans un chat, transmise a Codex, ni stockee dans un `.env` commite. Aucun fallback automatique vers alias, autre modele, autre region ou autre provider n'est autorise. `042b0` / `042b0b` ne livrent aucun runtime, adapter, SDK, secret, `.env`, appel IA, prompt runtime actif, golden set promu, production ou spec `043`, et ne debloquent pas `042b`.
