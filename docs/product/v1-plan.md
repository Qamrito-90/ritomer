...
### Active
- `specs/active/041-internal-poc-blockers-ux-readiness-v1.md`
...
- `041-internal-poc-blockers-ux-readiness-v1` est active pour cadrer les blockers POC issus du smoke `040` : statut cockpit plus comprehensible, mapping plus premium et scannable, rubriques Preuves / Justifications plus metier et francaises, montants Import formates en CHF, nettoyage des libelles techniques residuels, verification explicite des headers reseau et de l'absence IA runtime, puis smoke visuel post-correction. `041` reste strictement hors backend, DB/migration, OpenAPI, auth/JWT/proxy, nouvelle mutation, nouveau endpoint, IA runtime, GraphQL, export officiel, annexe legale finale, promesse CO/statutaire, secret, `.env`, token, credential et spec `042`.
