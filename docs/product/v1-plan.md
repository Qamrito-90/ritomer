...
### Rappels
- `042-controlled-ai-mapping-runtime-pilot-v1` est active en SPEC_CREATION DOCS_ONLY pour cadrer le premier pilote IA runtime reel strictement limite aux suggestions de mapping sur dossier demo synthetique. Aucun runtime, provider, backend, frontend, DB/migration, OpenAPI, CI, secret, `.env`, appel reseau IA, production, donnee cliente reelle ou spec `043` n'est introduit par cette activation documentaire.
- `030d runtime` provider reel general reste reporte hors pilote `042` : aucun provider IA reel, modele reel, SDK, prompt runtime actif, cout provider, appel reseau IA, microservice IA, GraphQL, RAG/vector store ou auto-apply ne sont actifs tant que les gates pre-code de `042` ne sont pas signes et qu'une implementation explicite n'est pas lancee.
