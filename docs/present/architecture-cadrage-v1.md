...
- Les capacites closes du present vont jusqu'a `017` : closing, import, mapping, controls, financial previews, workpapers, document storage, verification reviewer de la preuve, export pack audit-ready, et un frontend borne sur `/` puis `/closing-folders/:closingFolderId` pour l'entree dossiers, le detail dossier, l'import balance CSV, le mapping manuel unitaire et le cockpit controls.
