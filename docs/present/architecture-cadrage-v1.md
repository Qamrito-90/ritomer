...
- Les capacites closes du present vont jusqu'a `016` : closing, import, mapping, controls, financial previews, workpapers, document storage, verification reviewer de la preuve, export pack audit-ready, et un frontend borne sur `/` puis `/closing-folders/:closingFolderId` pour l'entree dossiers, le detail dossier, le cockpit controls et l'import balance CSV.
