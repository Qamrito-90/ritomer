...
- Le present UX inclut maintenant un frontend borne sur `/` et `/closing-folders/:closingFolderId`, avec entree dossiers, detail dossier, import balance CSV borne et cockpit controls/readiness read-only.
- Au-dela de ces surfaces bornees, la V1 reste backend-first dans son execution ; la verite UX du present couvre les capacites closes jusqu'a `016`.
