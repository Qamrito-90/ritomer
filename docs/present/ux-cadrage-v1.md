...
- Les surfaces UX visibles/frontend du present restent les surfaces closes jusqu'a `025`; `026` est une decompression frontend sans changement visible et `027-minimal-annex` ne cree aucune UX visible/frontend.
- Au-dela de ces surfaces bornees, la V1 reste backend-first dans son execution ; le repo vivant porte des capacites backend-documentaires closes jusqu'a `027`, dont `minimal-annex-v1` comme read-model backend non statutaire, non persiste et non exporte.
...
- une surface frontend ou UX visible d'annexe minimale
