...
- La spec 046 est l'unique spec active. M1.1A livre la fondation backend auth/tenant et M1.1B le kernel backend de session process-local, desactive par defaut. L'outcome final M1.1 n'est pas livre et C et D ne sont pas implementes.
- M1.1B ne modifie aucune surface frontend : aucun coordinator navigateur, ecran de login/logout, parcours OIDC partage ou integration de session distribuee n'est livre.
...
- M1 `AUTHENTICATED_PRODUCT_SHELL` reste une cible non livree : login, Home, Dossiers, workspace dossier, Activite IA, Parametres, contexte tenant/role et logout. M1.1A et M1.1B ne livrent aucun de ces comportements UX ; B fournit uniquement leur future frontiere backend locale.
- En developpement local, le kernel B permet au futur coordinator C de remplacer uniquement le passage par l'IdP externe. Il ne contourne jamais les autorisations serveur et utilise des utilisateurs synthetiques, memberships, roles et tenants existants ; tout environnement interne partage exigera un vrai OIDC en M1.2.
