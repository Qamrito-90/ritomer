...
class ControlsApiTest {
  ...
  @Test
  fun `controls endpoint requires authentication and emits no audit`() {
    mockMvc.get("/api/closing-folders/${UUID.randomUUID()}/controls") {
  ...
  @Test
  fun `controls endpoint returns 400 when tenant header is absent blank malformed and when id is invalid`() {
    ...
    mockMvc.get("/api/closing-folders/not-a-uuid/controls") {
  ...
  @Test
  fun `all current tenant roles can read controls`() {
    val tenantId = uuid("11111111-1111-1111-1111-111111111111")
    val closingFolder = seedClosingFolder(tenantId)
  ...
  @Test
  fun `controls returns 403 when tenant is inaccessible membership inactive or tenant inactive`() {
  ...
  @Test
  fun `controls returns 404 when closing is absent or outside tenant`() {
    val tenantAlphaId = uuid("11111111-1111-1111-1111-111111111111")
    val tenantBetaId = uuid("22222222-2222-2222-2222-222222222222")
    ...
    mockMvc.get("/api/closing-folders/${betaClosing.id}/controls") {
      header(ACTIVE_TENANT_HEADER, tenantAlphaId.toString())
      with(actorJwt("user-123"))
    }.andExpect { status { isNotFound() } }
  ...
  @Test
  fun `controls returns blocked with import next action when no import exists`() {
    val tenantId = uuid("11111111-1111-1111-1111-111111111111")
    val closingFolder = seedClosingFolder(tenantId)
    seedMembership("user-123", tenantId, TenantRole.ACCOUNTANT)
    ...
    mockMvc.get("/api/closing-folders/${closingFolder.id}/controls") {
      ...
      jsonPath("$.latestImportPresent") { value(false) }
      jsonPath("$.latestImportVersion") { value(nullValue()) }
      jsonPath("$.mappingSummary.total") { value(0) }
      jsonPath("$.mappingSummary.mapped") { value(0) }
      jsonPath("$.mappingSummary.unmapped") { value(0) }
      jsonPath("$.unmappedAccounts.length()") { value(0) }
      jsonPath("$.controls[0].code") { value("LATEST_VALID_BALANCE_IMPORT_PRESENT") }
      jsonPath("$.controls[0].status") { value("FAIL") }
      jsonPath("$.controls[1].code") { value("MANUAL_MAPPING_COMPLETE_ON_LATEST_IMPORT") }
      jsonPath("$.controls[1].status") { value("NOT_APPLICABLE") }
  ...
  @Test
  fun `controls returns blocked with ordered unmapped accounts when mapping is incomplete`() {
    val tenantId = uuid("11111111-1111-1111-1111-111111111111")
    val closingFolder = seedClosingFolder(tenantId)
    seedMembership("user-123", tenantId, TenantRole.ACCOUNTANT)
    seedImportVersion(
      ...
      listOf(
        ...
        BalanceImportLine(4, "9000", "Revenue", decimal("0.00"), decimal("175.00"))
      )
    )
    ...
    mockMvc.get("/api/closing-folders/${closingFolder.id}/controls") {
      ...
      jsonPath("$.mappingSummary.total") { value(3) }
      jsonPath("$.mappingSummary.mapped") { value(1) }
      jsonPath("$.mappingSummary.unmapped") { value(2) }
      jsonPath("$.unmappedAccounts.length()") { value(2) }
      jsonPath("$.unmappedAccounts[0].accountCode") { value("9000") }
      jsonPath("$.unmappedAccounts[0].accountLabel") { value("Revenue") }
      jsonPath("$.unmappedAccounts[1].accountCode") { value("0500") }
      jsonPath("$.unmappedAccounts[1].accountLabel") { value("Receivable") }
      jsonPath("$.controls[0].status") { value("PASS") }
      jsonPath("$.controls[1].status") { value("FAIL") }
  ...
  @Test
  fun `controls reflects only the latest import after reimport and stale mappings no longer project`() {
    val tenantId = uuid("11111111-1111-1111-1111-111111111111")
    val closingFolder = seedClosingFolder(tenantId)
    seedMembership("user-123", tenantId, TenantRole.ACCOUNTANT)
    seedImportVersion(
    ...
    seedImportVersion(
      ...
      2,
      listOf(
        BalanceImportLine(4, "3000", "Receivable", decimal("100.00"), decimal("0.00")),
        BalanceImportLine(2, "1000", "Cash", decimal("0.00"), decimal("100.00"))
      )
    )
    ...
    mockMvc.get("/api/closing-folders/${closingFolder.id}/controls") {
      ...
      jsonPath("$.mappingSummary.total") { value(2) }
      jsonPath("$.mappingSummary.mapped") { value(1) }
      jsonPath("$.mappingSummary.unmapped") { value(1) }
      jsonPath("$.unmappedAccounts.length()") { value(1) }
      jsonPath("$.unmappedAccounts[0].accountCode") { value("3000") }
      jsonPath("$.unmappedAccounts[0].accountLabel") { value("Receivable") }
      jsonPath("$.controls[0].code") { value("LATEST_VALID_BALANCE_IMPORT_PRESENT") }
      jsonPath("$.controls[0].status") { value("PASS") }
      jsonPath("$.controls[1].code") { value("MANUAL_MAPPING_COMPLETE_ON_LATEST_IMPORT") }
      jsonPath("$.controls[1].status") { value("FAIL") }
  ...
  fun `controls returns ready when import exists and mapping is complete`() {
    ...
    mockMvc.get("/api/closing-folders/${closingFolder.id}/controls") {
      ...
      jsonPath("$.latestImportVersion") { value(2) }
      jsonPath("$.mappingSummary.total") { value(2) }
      jsonPath("$.mappingSummary.mapped") { value(2) }
      jsonPath("$.mappingSummary.unmapped") { value(0) }
      jsonPath("$.unmappedAccounts.length()") { value(0) }
  ...
  @Test
  fun `controls allows archived closing read and marks next action as not actionable`() {
    val tenantId = uuid("11111111-1111-1111-1111-111111111111")
